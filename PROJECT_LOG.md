...
### [2026-08-10] — تغميق خلفية بوابة ولي الأمر

- **ما تم:** خلفية `ParentLayout` / `main` → `#DEE6EC` لإبراز البطاقات البيضاء؛ باقي الأدوار دون تغيير.

### [2026-08-10] — ثيم إيموجي لخدمات ولي الأمر (مثل وصول الطالب)

- **ما تم:** شبكة «الخدمات» بأيقونات إيموجي ملونة بنفس أسلوب بطاقات «الوصول السريع» للطالب (`variant="emoji"`).
- **الملفات:** `roleHomeIcons.ts` · `RoleHomeIconGrid.tsx` · `ParentDashboardPage.tsx`

### [2026-08-10] — توثيق جلسة 08-10 + تسمية المعلم المناوب 24/7

- **ما تم:**
  - توحيد التسمية إلى **المعلم المناوب 24/7** (أرقام إنجليزية).
  - وثائق: `docs/PLATFORM_STATUS_2026-08-10.md` · تحديث `docs/README.md` · `CHANGELOG.md`.
- **نشر الإنتاج (مرجع):**
  - `cbb2817` حساب + أفاتار
  - `44fc441` رئيسية ولي الأمر
  - `94239bd` أزرار ملف/خروج ظاهرة
  - `206ee4b` خصوصية طلاب أدمن المنطقة
  - `447281e` تسمية المناوب 24/7

### [2026-08-10] — إخفاء بيانات تواصل الطلاب عن أدمن المنطقة

- **ما تم:**
  - أدمن المنطقة يرى للطالب: الاسم + المواد المسجّلة فقط (بدون هاتف/إيميل/عنوان).
  - منع إنشاء طالب من لوحة الأدمن — عبر `/register` فقط؛ التفاصيل الكاملة للسوبر أدمن.
  - المالية تظهر بأسماء بدون هواتف الطلاب (اشتراكات/مشتريات/تسجيلات/ملف مساق/مواهب/تعيين مشرفين).
  - المعلمون وأولياء الأمور: بدون تغيير سياسة التواصل.
- **الملفات:** `AdminStudentPrivacy.php`، `Admin/UserController`، خدمات الاشتراك/التسجيل، صفحات أدمن الفرونت.

### [2026-08-10] — أزرار ظاهرة: الملف الشخصي + تسجيل الخروج لكل الأدوار

- **ما تم:**
  - `AccountMenu` يعرض زرّي الملف الشخصي وتسجيل الخروج دائماً (هيدر / سايدبار) بدل القائمة المنسدلة فقط.
  - توحيد التجربة: طالب، ولي أمر، معلم، مشرف، أدمن، سوبر أدمن.
  - محدد أبناء ولي الأمر يبقى منفصلاً في الرئيسية.
- **الملفات:** `AccountMenu.tsx`، layouts لكل الأدوار، `AppLayout` سايدبار.

### [2026-08-10] — بوابة ولي الأمر: بلا سايدبار + 4 مربعات + أيقونات 7 أعمدة

- **ما تم:**
  - حذف سايدبار ولي الأمر؛ هيدر + أيقونات فقط.
  - شبكة خدمات 7 أعمدة؛ دمج دوري/تحديات تحت أيقونة واحدة مع تبويبات.
  - إعادة تسمية المساعد الذكي → المعلم المناوب 24/7.
  - 4 مربعات: حضور مرئي، امتحانات/واجبات قادمة، ملاحظات معلم، تقرير تحسّن.
  - API: `GET /parent/children/{id}/insights`.
- **ما تبقى:** ملاحظات أثناء الحصة الحقيقية تحتاج مصدر بيانات لاحقاً.

### [2026-08-10] — قائمة حساب موحّدة + ملف شخصي + صورة لكل الأدوار

- **ما تم:**
  - مكوّن `AccountMenu`: النقر على الاسم/الأفاتار → الملف الشخصي + تسجيل الخروج.
  - ربط القائمة في: طالب، ولي أمر، معلم، مشرف، أدمن، سوبر أدمن.
  - صفحات بروفايل: `/student/profile`، `/teacher/profile`، `/admin/profile` + ربط إعدادات ولي الأمر/المشرف.
  - Backend: حقل `avatar_url` + `POST /auth/avatar` + تضمين الصورة في `userPayload`.
- **ما تبقى / ملاحظات:**
  - على الإنتاج: `php artisan migrate --force` ثم `php artisan storage:link` إن لم يكن موجوداً، ثم build الفرونت.
...
### [2026-07-13] — إزالة المحتوى الوهمي وربط الاعتمادات/الإشعارات

- **ما تم:**
  - اعتمادات السوبر أدمن من امتحانات/واجبات حقيقية (`/super-admin/approvals`)
  - إشعارات من `notifications/history`؛ دعم/دوري/أكاديمية/إرشاد/دردشة مشرف → empty
  - تقارير ولي الأمر من `/parent/children/{id}/report`
- **Commit:** `ce145b7`
...
### [2026-07-13] — تثبيت OTP للمرحلة الحالية (رمز ثابت + رقم اختبار)

- **ما تم:**
  - `OTP_FIXED_CODE` + `OTP_SHOW_IN_RESPONSE` — رمز ثابت يظهر في شاشة الدخول (الآن فقط)
  - `WASENDER_TEST_RECIPIENT` — توجيه OTP لرقم واتساب واحد إن وُجد المفتاح
  - تطبيع فلسطين `05…` → `00970` مع فحص 972
- **Commit:** `3efbe7d` ثم تحديث الرمز الثابت
- **على السيرفر:** إضافة المتغيرات في `.env` ثم `config:clear`
...
### [2026-07-13] — مسؤولو الدولة: إزالة دخول + إيميل/باسورد

- إزالة زر «دخول» (impersonate) من `CountryAdminsPage`
- إلزام البريد وكلمة المرور عند إضافة/تعديل أدمن الدولة

---

### [2026-07-13] — أسئلة شائعة ديناميكية (سوبر أدمن فقط)

- صفحة `/dashboard/faqs` + API `super-admin/faqs`
- ظهورها في الصفحة الرئيسية عبر `/public/faqs`
- إزالة إدارة FAQ من أدمن الدولة (CMS)
...
### [2026-07-19] — دوري الزملاء = دوريات 1v1 من الأدمن

- **ما تم:** تفعيل `/student/peer-league` بدل الصفحة الفارغة — يعرض دوريات الأدمن من نوع `1v1` فقط (انضمام + ترتيب XP)
- **API:** `GET /student/leagues?type=1v1` (فلتر اختياري)
- **الملفات:** `Student/LeagueController`, `leagueSlice`, `StudentPeerLeaguePage`
- **ملاحظة:** إن لم يظهر شيء، الأدمن ينشئ دورياً من «دوري ياقوت» بنوع منازلة فردية (1v1) ويفعّله
...
### [2026-07-19] — مكتبة الياقوت (أدمن + طالب)

- **ما تم:** جدول `library_items` (كتاب/إهداء/أسئلة سنوات/ملخص) حسب الدولة والصف؛ CRUD أدمن البلد؛ عرض الطالب المصفّى
- **صفحات:** `/admin/library`، `/student/library`
- **Deploy:** `migrate --force` ثم build

---

### [2026-07-19] — حاضنة المواهب

- **ما تم:** جدول `talents`؛ استبيان أول دخول ثم بروفايل قابل للتعديل؛ قائمة أدمن البلد
- **صفحات:** `/student/talents`، `/admin/talents`
- **Deploy:** migrate + build

---

### [2026-07-19] — إكمال باقي صفحات Stub (طالب + ولي أمر)

- **خطة:** صديق الدراسة، كبسولة زمنية، مرشد، تواصل معلم، غرفة 24/7، إرشاد ولي، أكاديمية آباء، إنجازات، دوري أولياء
- **جداول:** `study_buddy_sessions`, `time_capsules`, `counseling_requests`, `parent_academy_items`
- **أدمن:** `/admin/counseling`, `/admin/parent-academy`

---

### [2026-07-19] — إصلاح جودة الواجهة + صفحات مضلِّلة

- **ما تم:**
  - زر «تسجيل الدخول» في الهبوط → `/login` (بدل نموذج الحجز)
  - إزالة رسائل الطالب من القائمة؛ `/student/messages` → تواصل المعلم
  - Migration: إصلاح لبنان (`LB` / `+961`) + بذر FAQ منصة إن فارغة
  - ربط مؤشر التطور والحضور لولي الأمر بتقرير الابن الحقيقي
  - جدول المعلم من الحصص المباشرة؛ حضور المعلم ومشرف الواجبات/الاختبارات بحالة فارغة صادقة
  - تحسين تباين زر الدخول المعطّل
- **Commits:** `2c97928`, `0dab7be`

---

### [2026-07-19] — دليل استخدام HTML برابط خاص

- **ما تم:** ملف `docs/دليل-استخدام-منصة-الياقوت.html` + نشره على  
  `https://alyaqoutgroup.net/internal/docs/yg-3bb4b9c226a4.html`
- **Commits:** `779068c`, `714dcc5`

---

### [2026-07-19] — إعادة تصميم الصفحة الرئيسية

- **ما تم:** هيرو مزيج (صف + واجهة)، نصوص ثقة، مسارات 3 أدوار، إثبات منتج؛ إحصاءات API فقط بعد عتبة نضج؛ placeholders بمقاسات التصميم في `public/landing/`
- **توثيق:** `docs/LANDING_PAGE.md`, `docs/PLATFORM_STATUS_2026-07-19.md`, `docs/README.md`
- **Commit:** `b0b0cf7`
- **ما تبقى:** رفع الصور الفعلية؛ CMS هبوط (مرحلة لاحقة)
