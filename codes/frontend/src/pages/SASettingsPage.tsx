          ...
          {activeTab==='platform'&&(
            <div style={card()}>
              ...
              <div style={{display:'flex',gap:10,marginTop:8}}>
                <button style={{padding:'11px 24px',borderRadius:12,background:C.goldGrad,color:'#1B2038',fontWeight:800,fontSize:13,border:'none',cursor:'pointer',boxShadow:'0 3px 10px rgba(201,149,42,0.3)'}}>💾 حفظ التغييرات</button>
              ...
              {['الرياض','جدة','الدمام','مكة المكرمة','المدينة المنورة','الطائف','أبها'].map((city,i)=>(
                <div key={i} style={{display:'flex',alignItems:'center',justifyContent:'space-between',padding:'12px 14px',borderRadius:12,border:`1px solid ${C.border}`,marginBottom:10}}>
                  ...
                  <div style={{display:'flex',gap:8}}>
                    <button style={{padding:'6px 14px',borderRadius:9,border:`1px solid ${C.border}`,background:'transparent',color:C.sub,fontSize:12,cursor:'pointer'}}>تعديل</button>
                    <button style={{padding:'6px 14px',borderRadius:9,border:'none',background:C.goldBg,color:C.gold,fontSize:12,fontWeight:700,cursor:'pointer'}}>إدارة</button>
                  </div>
                </div>
              ))}
              <button style={{width:'100%',padding:'11px',borderRadius:12,background:C.goldGrad,color:'#1B2038',fontWeight:800,fontSize:13,border:'none',cursor:'pointer',marginTop:8}}>+ إضافة منطقة جديدة</button>
            </div>
          )}
            ...
            <div style={card()}>
              ...
              {[
                ...
                <div key={i} style={{display:'flex',alignItems:'center',gap:12,padding:'14px',borderRadius:13,border:`1px solid ${C.border}`,marginBottom:10}}>
                  ...
                  <button style={{padding:'7px 14px',borderRadius:9,border:`1px solid ${C.border}`,background:'transparent',color:C.sub,fontSize:12,cursor:'pointer',flexShrink:0}}>{s.status==='متصل'?'إعادة الضبط':'ربط'}</button>
                </div>
              ))}
            </div>
          )}
