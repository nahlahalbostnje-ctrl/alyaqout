...
const C = {
  ...
  bg:'#0D1E3A', card:'#132540', navy:'#0D1E3A',
  text:'#FFFFFF', sub:'rgba(255,255,255,0.55)', border:'rgba(255,255,255,0.1)',
...
const inp = ():React.CSSProperties=>({background:'rgba(255,255,255,0.07)',border:`1.5px solid ${C.border}`,color:C.text,borderRadius:12,padding:'9px 14px',fontSize:13,width:'100%',outline:'none',fontFamily:"'Cairo',sans-serif",boxSizing:'border-box'});
...
const TYPE_OPTS = ['متابعة طالب','خطة تدخل','جلسة إرشاد','تقرير أداء','اجتماع','أخرى'];

interface Item { id:number; title:string; description?:string; type?:string; priority:string; status:string; due_date?:string; }
const EMPTY:Omit<Item,'id'> = { title:'', description:'', type:'متابعة طالب', priority:'medium', status:'pending', due_date:'' };

export default function SupervisorPersonalItemsPage() {
  ...
  const load = () => { setLoading(true); api.get('/supervisor/my-items').then(r=>setItems(r.data.items||[])).finally(()=>setLoading(false)); };
  useEffect(load,[]);
  ...
  const save = async () => {
    ...
    setSaving(true);
    try {
      if(editing) await api.put(`/supervisor/my-items/${editing.id}`, form);
      else        await api.post('/supervisor/my-items', form);
      setModal(false); load();
  ...
  return (
    <SupervisorLayout>
      <div style={{padding:'28px 32px',fontFamily:"'Cairo',sans-serif",direction:'rtl',background:'#0D1E3A',minHeight:'100vh'}}>

        <div style={{display:'flex',alignItems:'center',justifyContent:'space-between',marginBottom:24}}>
          <div>
            <h1 style={{color:C.text,fontWeight:900,fontSize:22,margin:0}}>خطط المتابعة الخاصة</h1>
            <p style={{color:C.sub,fontSize:13,marginTop:4}}>سجلاتك وخططك الإشرافية الشخصية</p>
          </div>
          <button onClick={openAdd} style={{display:'flex',alignItems:'center',gap:8,padding:'10px 22px',borderRadius:14,background:C.goldG,color:'#1B2038',fontWeight:800,fontSize:14,border:'none',cursor:'pointer',boxShadow:`0 4px 14px ${C.gold}40`}}>
        ...
        <div style={card({marginBottom:18,padding:'10px 14px',display:'flex',gap:8,flexWrap:'wrap'})}>
          {['الكل','قيد الانتظار','جارٍ','مكتمل','عالي','متوسط','منخفض'].map(f=>(
            <button key={f} onClick={()=>setFilter(f)} style={{padding:'5px 13px',borderRadius:20,fontSize:12,fontWeight:600,cursor:'pointer',background:filter===f?C.goldG:'transparent',color:filter===f?'#1B2038':C.sub,border:filter===f?'none':`1px solid ${C.border}`}}>{f}</button>
          ))}
        </div>
        ...
        : visible.length===0 ? (
          <div style={card({textAlign:'center',padding:60})}>
            ...
            <p style={{color:C.sub,fontSize:15}}>لا توجد خطط بعد — أضف أول متابعة!</p>
          </div>
        ) : (
            ...
            {visible.map(it=>(
              <div key={it.id} style={card({padding:'18px',borderRight:`4px solid ${PRIORITY_COLOR[it.priority]||C.border}`})}>
                <div style={{display:'flex',alignItems:'flex-start',justifyContent:'space-between',marginBottom:8}}>
                  <div style={{flex:1}}>
                    <div style={{display:'flex',gap:6,marginBottom:4}}>
                      ...
                      <span style={{padding:'1px 8px',borderRadius:20,fontSize:11,fontWeight:700,background:`${PRIORITY_COLOR[it.priority]}25`,color:PRIORITY_COLOR[it.priority]}}>{PRIORITY_LABEL[it.priority]}</span>
                    </div>
                ...
                <div style={{display:'flex',alignItems:'center',justifyContent:'space-between'}}>
                  <span style={{padding:'3px 10px',borderRadius:20,fontSize:11,fontWeight:700,background:`${STATUS_COLOR[it.status]}22`,color:STATUS_COLOR[it.status]}}>{STATUS_LABEL[it.status]}</span>
      ...
      {modal&&(
        <div style={{position:'fixed',inset:0,background:'rgba(0,0,0,0.6)',zIndex:200,display:'flex',alignItems:'center',justifyContent:'center'}} onClick={()=>setModal(false)}>
          <div style={card({width:480,maxWidth:'90vw',padding:28})} onClick={e=>e.stopPropagation()}>
            ...
            <div style={{display:'flex',flexDirection:'column',gap:14}}>
              ...
              <div>
                ...
                <textarea rows={3} value={form.description} onChange={e=>setForm(f=>({...f,description:e.target.value}))} placeholder="تفاصيل الخطة..." style={{...inp(),resize:'none'}} />
              </div>
            </div>
            <div style={{display:'flex',gap:10,marginTop:20}}>
              <button onClick={save} disabled={saving} style={{flex:1,padding:'11px',borderRadius:12,background:C.goldG,color:'#1B2038',fontWeight:800,fontSize:14,border:'none',cursor:'pointer',opacity:saving?0.7:1}}>
                {saving?'جاري الحفظ...':editing?'حفظ':'إضافة'}
              </button>
              <button onClick={()=>setModal(false)} style={{flex:1,padding:'11px',borderRadius:12,background:'transparent',color:C.sub,fontWeight:600,fontSize:14,border:`1px solid ${C.border}`,cursor:'pointer'}}>إلغاء</button>
            </div>
          </div>
        </div>
      )}
