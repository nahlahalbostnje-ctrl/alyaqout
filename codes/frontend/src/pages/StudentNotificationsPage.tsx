...
export default function StudentNotificationsPage() {
  ...
  return (
    <StudentLayout>
      <div dir="rtl" style={{ padding:20, fontFamily:"'Cairo',sans-serif", minHeight:'100%' }}>

        <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between', marginBottom:20, flexWrap:'wrap', gap:10 }}>
          <div>
            <div style={{ display:'flex', alignItems:'center', gap:8, marginBottom:4 }}>
              <div style={{ width:4, height:22, borderRadius:2, background:C.goldGrad }} />
              <h1 style={{ color:C.text, fontWeight:900, fontSize:20, margin:0 }}>الإشعارات 🔔</h1>
            </div>
            <p style={{ color:C.sub, fontSize:12.5, margin:0 }}>كل التذكيرات والتحديثات الخاصة بك في مكان واحد</p>
          </div>
          {unreadCount > 0 && (
            <button onClick={() => dispatch(markAllNotificationsRead())} style={{
              padding:'9px 18px', borderRadius:12, background:C.goldGrad, border:'none',
        ...
        <div style={{ background:C.card, borderRadius:16, boxShadow:C.shadow, border:`1px solid ${C.border}`, overflow:'hidden' }}>
          ...
          {!loading && items.map((n, i) => (
            <div key={n.id}
              ...
              <div style={{ width:36, height:36, borderRadius:10, background: n.is_read ? '#F3F4F6' : C.goldBg, display:'flex', alignItems:'center', justifyContent:'center', fontSize:17, flexShrink:0 }}>
                {TYPE_ICON[n.type] ?? '🔔'}
              </div>
              <div style={{ flex:1, minWidth:0 }}>
                <div style={{ display:'flex', alignItems:'center', gap:8 }}>
                  <p style={{ color:C.text, fontWeight:700, fontSize:13, margin:0 }}>{n.title}</p>
                  {!n.is_read && <span style={{ width:7, height:7, borderRadius:'50%', background:C.gold, flexShrink:0 }} />}
                </div>
                <p style={{ color:C.sub, fontSize:12, lineHeight:1.6, margin:'4px 0', whiteSpace:'pre-line' }}>{n.body}</p>
                <p style={{ color:C.dim, fontSize:10.5 }}>{timeAgo(n.created_at)}</p>
              </div>
            </div>
          ))}
        </div>
