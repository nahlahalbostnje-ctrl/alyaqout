...
const C = {
  ...
  bg:'#FAF6EE', card:'#FFFFFF', navy:'#0D1E3A',
...
const TYPE_OPTS = ['تذكير درس','مهمة تحضير','ملاحظة طالب','هدف تعليمي','اجتماع','أخرى'];

interface Item { id:number; title:string; description?:string; type?:string; priority:string; status:string; due_date?:string; }
const EMPTY:Omit<Item,'id'> = { title:'', description:'', type:'تذكير درس', priority:'medium', status:'pending', due_date:'' };

export default function TeacherPersonalItemsPage() {
  ...
  const load = () => { setLoading(true); api.get('/teacher/my-items').then(r=>setItems(r.data.items||[])).finally(()=>setLoading(false)); };
  useEffect(load,[]);
  ...
  const save = async () => {
    ...
    setSaving(true);
    try {
      if(editing) await api.put(`/teacher/my-items/${editing.id}`, form);
      else        await api.post('/teacher/my-items', form);
      setModal(false); load();
  ...
  const confirmPendingDelete = async () => {
    ...
    setDeleteError(null);
    try {
      await api.delete(`/teacher/my-items/${pendingDelete.id}`);
    ...
    <TeacherLayout>
      <div style={{padding:'28px 32px',fontFamily:"'Cairo',sans-serif",direction:'rtl'}}>
        ...
        <div style={{display:'flex',alignItems:'center',justifyContent:'space-between',marginBottom:24}}>
          <div>
            <h1 style={{color:C.text,fontWeight:900,fontSize:22,margin:0}}>مذكراتي ومهامي</h1>
            <p style={{color:C.sub,fontSize:13,marginTop:4}}>قائمتك الخاصة — مخصصة لك وحدك</p>
          </div>
        ...
        : visible.length===0 ? (
          <div style={card({textAlign:'center',padding:60})}>
            ...
            <p style={{color:C.sub,fontSize:15}}>لا توجد عناصر — أضف أول مهمة!</p>
          </div>
        ) : (
          ...
          <div style={card({width:480,maxWidth:'90vw',padding:28})} onClick={e=>e.stopPropagation()}>
            ...
            <div style={{display:'flex',flexDirection:'column',gap:14}}>
              <div>
                ...
                <input value={form.title} onChange={e=>setForm(f=>({...f,title:e.target.value}))} placeholder="عنوان المهمة..." style={inp()} />
              </div>
