...
export default function SupervisorLayout({ children }: { children: ReactNode }) {
  ...
  return (
    <div style={{ display:'flex', minHeight:'100vh', background:'#F5EDD8', fontFamily:"'Cairo',sans-serif", direction:'rtl' }}>
      ...
      {/* Sidebar */}
      <aside style={{ width:240, flexShrink:0, display:'flex', flexDirection:'column', height:'100vh', position:'sticky', top:0, background:'#0D1E3A', borderLeft:'1px solid rgba(255,255,255,0.07)' }}>
      ...
      <div style={{ flex:1, display:'flex', flexDirection:'column', minWidth:0 }}>
        <header style={{ height:54, display:'flex', alignItems:'center', justifyContent:'space-between', padding:'0 24px', background:'#fff', borderBottom:'1px solid #EDE3CE', boxShadow:'0 1px 4px rgba(0,0,0,0.05)', flexShrink:0 }}>
          <div style={{ display:'flex', alignItems:'center', gap:6, color:'#9CA3AF', fontSize:12.5 }}>
            <span>بوابة المشرف</span>
            <span>›</span>
            <span style={{ color:'#1B2038', fontWeight:700 }}>{pageLabel}</span>
          </div>
