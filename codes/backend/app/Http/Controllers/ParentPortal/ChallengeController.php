...
class ChallengeController extends Controller
{
    ...
    public function index(): JsonResponse
    {
        ...
        $items = Challenge::where('parent_id', $parentId)
            ->where('type', 'family')
    ...
    public function store(Request $request): JsonResponse
    {
        ...
        $parent  = Auth::user();
        $student = User::findOrFail((int) $data['student_id']);

        $challenge = $this->challenges->createFamilyByParent($parent, $student, $data);

        return response()->json([
            'message'   => 'تم إنشاء التحدي العائلي',
            'challenge' => $this->challenges->format($challenge),
        ], 201);
    }

    public function accept(Challenge $challenge): JsonResponse
    {
        $updated = $this->challenges->accept($challenge, Auth::user());

        return response()->json([
            'message'   => 'تمت الموافقة على التحدي',
            'challenge' => $this->challenges->format($updated),
        ]);
    }

    public function reject(Challenge $challenge): JsonResponse
    {
        $updated = $this->challenges->reject($challenge, Auth::user());
    ...
    {
        abort_unless((int) $challenge->parent_id === (int) Auth::id(), 403);
        ...
        return response()->json([
            'message'   => $updated->status === 'completed' ? 'أُكمل التحدي العائلي' : 'تم تسجيل التقدم',
            'challenge' => $this->challenges->format($updated),
        ]);
    }
